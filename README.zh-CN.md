...
**编码 agent 的意图对齐引擎（轻量插件）**：把需求意图断层的消解前置到编码之前——
PRD 经意图置信度评估 → 断层按三级漏斗消解 → 产出带技术打标的 mermaid 契约
（状态机/时序图/决策表）→ lint 机械门禁归零 → 才允许编码开工。

以 **MCP server（stdio 子进程）** 形态运行，Claude Code / Pi agent 等即插即用，
**无常驻服务、无长连接、无外部凭据**，纯 stdio 即可零依赖跑通全流程。

> v0.2.0 起钉钉交互（群共识通道 + 阻塞式决策闸门）剥离为姊妹篇
> [**intent-gate-service**](https://github.com/baixinghao/intent-gate-service)（独立 MCP 服务）。intent-gate 回归轻量：
> 只保留意图对齐与需求解析，钉钉一概不碰。

## 为什么需要它

代码幻觉的重要来源不是模型能力，是**意图断层在编码阶段被注意力掩盖、被猜测填补**：
编码时 agent 的注意力集中在代码生成上，PRD 里"只有成功没有失败流""防重复无服务端方案"
这类断层，会被顺手猜一个合理实现混过去。

宿主自带的意图弹窗（AskUserQuestion 之类）解决不了这个问题：判断是临时的、
答案是易逝的——上下文一压缩、会话一结束，对齐现场就没了。

intent-gate 的对策是**判断归宿主、纪律归代码**：
